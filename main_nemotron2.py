...
if __name__ == "__main__":
    ...
    for embedding_model in embedding_models:
        for strategy in strategies:
            ...
            def process_query(query_id, query, tbl, base_prompt, llm, embedding_model, strategy):
            ...
            with ThreadPoolExecutor(max_workers=5) as executor:  #pour le nombre de threads
                # [ Process queries in parallel ]
                futures = []
                for query_id, query in enumerate(queries):
                    tbl = collection.open_table(f"chunks_{embedding_model}_{strategy}")
                    futures.append(executor.submit(process_query, query_id, query, tbl, base_prompt, llm, embedding_model, strategy))
